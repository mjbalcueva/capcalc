		...
		<header className="relative z-[999]">
			...
			<nav className="fixed left-1/2 top-[0.15rem] flex h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
				<ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-screen sm:flex-nowrap sm:gap-5">
					...
					/>
					<MenuItem
						menuItem={menuItems[1] as { content: string; hash: string }}
						activeSection={activeSection}
						setActiveSection={setActiveSection}
						setTimeOfLastClick={setTimeOfLastClick}
					/>
					<MenuItem
						menuItem={menuItems[3] as { content: string; hash: string }}
						activeSection={activeSection}
						setActiveSection={setActiveSection}
						setTimeOfLastClick={setTimeOfLastClick}
					/>
					<MenuItem
						menuItem={menuItems[2] as { content: string; hash: string }}
						activeSection={activeSection}
						setActiveSection={setActiveSection}
						setTimeOfLastClick={setTimeOfLastClick}
					/>
...
}> = ({ menuItem, activeSection, setActiveSection, setTimeOfLastClick }) => {
	return (
		<motion.li
