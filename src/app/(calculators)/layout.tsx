...
import { inputSchema as eccentricLoadingSchema } from '@/lib/schemas/eccentric-loading'
...
const pageToSchemaMapping = {
	...
	'/eccentric-loading': eccentricLoadingSchema,
...
export default function Layout({ children }: { children: React.ReactNode }) {
	...
	return (
		<div className="relative min-h-screen bg-white antialiased dark:bg-[#09090b]">
			...
			>
				<section className="flex flex-col gap-4 text-center sm:gap-4 sm:text-start md:gap-6">
					<h1 className="h-14 bg-opacity-50 bg-gradient-to-b from-muted-foreground/90 to-foreground/90 bg-clip-text text-3xl font-bold text-transparent drop-shadow-md dark:from-muted-foreground/90 dark:to-foreground/90 sm:text-4xl md:text-5xl">
						{activeCalculatorItem?.title}
					</h1>
					<p className="max-w-5xl text-muted-foreground drop-shadow-sm sm:text-lg">
